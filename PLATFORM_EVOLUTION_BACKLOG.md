...
| HACS pull-request validation reliability (`HACS-CI-PR-REF-001`) | P1 | Reassessed — no workflow change | `docs/software_assurance/HACS_CI_VALIDATOR_EVIDENCE_RECONCILIATION_ASSESSMENT.md`; PR #459 HACS success for both head and merge commits; existing HACS action pinning exception | retain HACS validation for pull requests and canonical `main`; preserve future failed-job evidence before any new assessment; no Runtime, qualification, gate or action-pinning change |
