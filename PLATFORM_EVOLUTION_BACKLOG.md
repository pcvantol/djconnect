...
| HACS pull-request validation reliability (`HACS-CI-PR-REF-001`) | P1 | Planned | `docs/software_assurance/HACS_CI_FAILURE_CLASSIFICATION_ASSESSMENT.md`, existing HACS action pinning exception and a separate approved workflow-correction prompt | retain HACS validation on canonical `main`; no Runtime, qualification, gate or action-pinning change without bounded implementation evidence |
