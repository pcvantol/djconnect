...
Phase 9L-R4 has run and is not yet qualified. The active next step is the
narrow Phase 9L-R5 Docker Desktop operator reset. The modular local HA
...
`PROFILE-005` select `ha-profile`. Phase 9L-R4 failed the stable Docker gate
before HA lab startup: the first no-mount Home Assistant image `/bin/true`
probe remained in `Created` with Docker events showing `create`/`attach` but
no `start`/`die`. The blockers are recorded in
`docs/verification/reports/PHASE_09L_LOCAL_HA_VERIFICATION_LAB.md`.

Use this clean-session prompt:

```text
Read BOOTSTRAP_CODEX_VERIFICATION.md and execute Phase 9L-R5 from PROMPT_INDEX.md after performing an operator-approved Docker Desktop reset or reinstall.
```
...
| 9L-R5 | Docker Desktop Operator Reset And Local HA Lab Qualification | Active | `prompts/verification/PHASE_09L_R5_DOCKER_DESKTOP_OPERATOR_RESET.md` | Phase 9L-R4 not qualified; no-mount probe remains in Created | Update `docs/verification/reports/PHASE_09L_LOCAL_HA_VERIFICATION_LAB.md`; evidence under `artifacts/verification/evidence/` | New remediation PR | Phase 9V rerun only after lab qualification |
