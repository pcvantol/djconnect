...
dedicated `djconnect-verification-ha` container. Lab-only Home Assistant auth
bootstrap has been added, so a fresh lab can generate its own verification
credentials before REST/WebSocket qualification. The blockers are recorded in
`docs/verification/reports/PHASE_09L_LOCAL_HA_VERIFICATION_LAB.md`.

Use this clean-session prompt:

```text
Read BOOTSTRAP_CODEX_VERIFICATION.md and execute Phase 9L-R from PROMPT_INDEX.md after restoring local Docker.
```
...
- `Not qualified - external Docker prerequisite` means remediation code
  improved framework behavior, including lab-only HA auth bootstrap, but live
  lab qualification is blocked by local Docker runtime prerequisites outside
  repository code.
