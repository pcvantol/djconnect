...
- Ask DJ clear synchronization uses `POST /api/djconnect/ask_dj/clear`, `POST /api/djconnect/ask_dj/history_state` and the matching services. A clear increments the server-side generation and history-state returns `ask_dj_clear_required` for clients that still have an older generation.
