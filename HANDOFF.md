...
- Ask DJ / DJ Memory is server-side in the Home Assistant integration. iOS, macOS, watchOS and Raspberry Pi clients remain lightweight and store no DJ Memory. HA keeps runtime session history plus persistent Store data under `djconnect_memory` version `1`, keyed by HA user id when available and otherwise by DJConnect device/client id. ESP32 does not get Ask DJ chat UI/history; it keeps the existing voice/playback command flow. Do not store OAuth tokens, bearer tokens, raw audio or full prompts in memory.
- Text chat for app/display Ask DJ uses `POST /api/djconnect/ask_dj/message` from iOS, macOS, watchOS and Raspberry Pi; service `djconnect.ask_dj` remains a developer entrypoint. The backend classifies informational questions separately from playback/device actions, so questions such as "Waarom koos je dit nummer?" must not change playback while requests such as "Volgende nummer" or "Zet rustige muziek op" can execute Spotify/Home Assistant actions and return a DJ response.
- Cross-device Ask DJ history is stored in Home Assistant Store key `djconnect_ask_dj_history` version `1`, scoped by HA user id. iOS, macOS, Apple Watch and Raspberry Pi clients use `GET /api/djconnect/ask_dj/history?since_revision=<number>` and `POST /api/djconnect/ask_dj/history/clear` to reconcile local cache. `client_message_id` dedupes retries; `client_id` and `client_type` are metadata only.
...
- Apple push notifications are server-side, optional and best-effort for `ios`, `macos` and `watchos`. Clients register with authenticated `POST /api/djconnect/push/register` and unregister with `POST /api/djconnect/push/unregister`; registrations are stored per HA user/account, `device_id`, `client_type` and APNs token. Push payloads are wake/sync hints only and must never include Spotify/HA tokens, raw prompts, raw LLM context, full memory/history or long/raw assistant text. Clients must sync through `/ask_dj/history` after opening.
- APNs delivery uses provider token auth with `.p8` credentials from `APNS_TEAM_ID`, `APNS_KEY_ID`, `APNS_PRIVATE_KEY` or `APNS_PRIVATE_KEY_PATH`, topic env vars per Apple platform and `APNS_ENVIRONMENT` (`sandbox` default, `production` for release). Missing credentials keep push disabled without breaking normal Ask DJ flow; invalid APNs tokens are marked disabled/invalid.
