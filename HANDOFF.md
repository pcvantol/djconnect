...
- Numeric Ask DJ mood values are still accepted for backwards compatibility, but HA derives the canonical `mood_zone`: Chill for `0`-`24`, Groove for `25`-`59`, Energy for `60`-`84` and Party for `85`-`100`. Ask DJ prompts, DJ Memory context and profile/recommendation text should use the zone name plus persona hint, not only the number.
