// Minimal DJConnect device-side raw WAV voice request example.
// ESP firmware records microphone audio as WAV and uploads it to the DJConnect
...
bool uploadWavToDJConnect(const uint8_t* wav, size_t wavLen) {
  ...
  http.addHeader("X-DJConnect-Device-ID", DEVICE_ID);
  http.addHeader("Content-Type", "audio/wav");

  int code = http.POST((uint8_t*)wav, wavLen);
  if (code < 200 || code >= 300) {
    Serial.printf("DJConnect WAV upload failed: HTTP %d\n", code);
