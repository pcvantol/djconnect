...
class DevOnboardingScriptTests(unittest.TestCase):
    ...
    def test_macos_host_bootstrap_has_no_token_argument(self) -> None:
        ...
        self.assertIn("onboarding.package_version: 4.4.0", desired_state)
        ...
        self.assertIn("tailscale set --auto-update", SCRIPT.read_text(encoding="utf-8"))
        ...
        onboarding_source = SCRIPT.read_text(encoding="utf-8")
        self.assertIn("seed_home_assistant_lab_baselines", onboarding_source)
        self.assertIn("home_assistant_lab/configuration.yaml", onboarding_source)
        self.assertIn("Preserving existing Home Assistant configuration", onboarding_source)
    ...
    def test_macos_host_bootstrap_reports_its_semantic_version(self) -> None:
        ...
        self.assertEqual(
            result.stdout,
            "DJConnect macOS Development Host Bootstrap 2.0.16\n",
        )
        ...
        self.assertIn("## [2.0.5] - 2026-07-16", changelog)
...
""",
    ...
    def test_macos_host_bootstrap_uses_a_thin_packaged_entry_point(self) -> None:
        ...
        self.assertIn("package.version: 2.0.16", manifest)
        ...
        self.assertIn("component.operations.version: 1.3.8", manifest)
        ...
        self.assertIn("require_canonical_onboarding_4_4_0", source)
        self.assertIn("requires onboarding 4.4.0", source)
    ...
    def test_macos_host_bootstrap_preserves_its_active_source_checkout_during_repair(self) -> None:
        ...
        self.assertIn('"$(cd "$directory" && pwd -P)" == "$REPOSITORY_ROOT"', source)
        self.assertIn("Preserving active host-bootstrap checkout", source)
        self.assertLess(
    ...
    def test_windows_runner_recovery_bootstrap_keeps_tokens_off_the_cli(self) -> None:
        ...
        self.assertIn("Set-ServiceVirtualAccountLogon", source)
        self.assertIn("$configArguments", source)
        self.assertIn("not a password-bearing user account", source)
        self.assertNotIn('password= `"`"', source)
        self.assertIn("Start-Process -FilePath $scExe", source)
