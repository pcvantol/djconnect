...
- Als Ask DJ history door de limiet van 200 berichten wordt getrimd, geeft de backend `history_limit`, `history_trimmed_before` en `history_trimmed_count` terug en voegt maximaal één assistant system message toe met `message_kind:"system"`, `origin:"history_retention"` en intent `history_limit_reached`. Clients moeten lokale berichten ouder dan `history_trimmed_before` verwijderen en niet op tekstinhoud parsen.
