...
- Backend playback loopt via de HA integration en wordt user-facing aangeboden als optionele/native `media_player` proxy; device-instellingen lopen via `POST /api/device/command`. De playback proxy en Spotify-backed HA entities moeten live backend snapshots gebruiken voor play/pause, current track, album art, volume, output/source, repeat, shuffle, queue, playlists en playback availability.
...
- Actuele integratieversie: `3.1.81`.
