...
class RepairsTest(unittest.TestCase):
    ...
    def test_spotify_repair_prerequisites_accept_entry_options(self) -> None:
        entry = types.SimpleNamespace(
            entry_id="entry-1",
            data={},
            options={
    ...
    def test_spotify_reauth_fix_flow_creates_oauth_pending_context(self) -> None:
        ...
        self.assertEqual(
            result["description_placeholders"]["title"],
            "DJConnect opnieuw autoriseren bij Spotify",
        )
        self.assertIn(
            "Spotify toestemming",
            result["description_placeholders"]["description"],
        )
