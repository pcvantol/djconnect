...
| VPB-045 | P1 | Windows runtime environment follow-up | Phase 13E executed `WIN-001` and failed closed in run `djv-20260712T121332Z-a50bf9b10e` because `DJCONNECT_VERIFICATION_WINDOWS_TARGET_JSON` and real `pcvantol/djconnect-windows` artifact/runtime commands were not configured. Provide the target JSON with commands against the Parallels Windows VM or approved local Windows target, then rerun `WIN-001` with persisted evidence. | Windows Adapter / Execution Environment / Operator | `djconnect`, `djconnect-windows` | Blocks live Windows scenario pass and Phase 14 | M | Phase 13E-R Windows live target configuration remediation |
