...
| VPB-032 | P0 | Execution Environment issue | Apple verification now has to keep the Xcode/iOS simulator platform current and qualify only the latest eligible iOS simulator runtime for the active mode. iOS 27.0 is beta on 2026-07-11 and is excluded from default stable qualification; it remains available only through the `future_beta` route. The integrated future-beta iOS 27.0 qualification timed out in the XCTest UI healthcheck after build, install, launch, screenshot and logs passed. | Verification Execution Environment / Apple Adapter / XCTest runner | `djconnect`, `djconnect-app` | Blocks Phase 10E retry and broad Apple scenario execution | M | Phase 10E-R2 |
