...
| VPB-005 | P0 | HA Adapter gap | Live websocket transport remains unqualified until the safe HA Docker runtime is running and lab auth can produce a token. | HA Adapter / Verification Environment | `djconnect` | Blocks live HA qualification | M | Phase 9V rerun |
...
| VPB-016 | P0 | Environment issue | Phase 9L-R2 proved the remaining blocker is Docker Desktop/containerd container-start health: a no-mount `docker run` probe using a local image remained `Created`, and Docker Desktop restart returned HTTP 500/hung readiness checks. | Local Docker / Operator | Local workstation | Blocks Phase 9V rerun | S | Phase 9L-R3 Docker Desktop repair |
| VPB-017 | P0 | Environment issue | Phase 9L-R3 restored Docker server metadata and one no-mount HA image probe started successfully, but subsequent HA image starts again remained in `Created` with no `start` event. The canonical `ha-profile` lab therefore still cannot qualify. | Local Docker / Operator | Local workstation | Blocks Phase 9V rerun | S | Phase 9L-R4 Docker Desktop clean runtime repair |
| VPB-018 | P0 | Environment issue | Phase 9L-R4 failed the stable Docker gate on probe 1: the no-mount HA image `/bin/true` container remained in `Created`, with Docker events showing create/attach but no start/die. | Local Docker / Operator | Local workstation | Blocks Phase 9V rerun | S | Operator-approved Docker Desktop runtime reset or reinstall |
| VPB-019 | P0 | Environment issue | Phase 9L-R5 passed repeated no-mount probes after `docker desktop restart`, but the canonical HA lab and a direct bind-mount probe remained in `Created`. A blocking macOS permission popup for Docker Desktop access to `Documents` was identified after R5 and matches the repository bind-mount failure. | Local Docker / Operator | Local workstation | Blocks Phase 9V rerun | S | Approve Docker Desktop access to `Documents`, then rerun bind-mount and lab qualification gates; use Docker Desktop purge/factory reset/reinstall only if permission remediation fails |
