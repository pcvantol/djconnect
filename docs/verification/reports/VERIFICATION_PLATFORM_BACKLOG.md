...
| VPB-019 | P0 | Environment issue | Phase 9L-R5 passed repeated no-mount probes after `docker desktop restart`, but the canonical HA lab and a direct bind-mount probe remained in `Created`. The remaining blocker is Docker Desktop file-sharing/bind-mount startup for repository paths. | Local Docker / Operator | Local workstation | Blocks Phase 9V rerun | S | Operator-approved Docker Desktop Clean/Purge data, factory reset or reinstall |
