...
The canonical interpreter for this incident is the declared development-host
Python 3.12 executable, `/opt/homebrew/bin/python3.12`. Each qualification
record must state its resolved version and use that exact executable for its
focused and full-suite comparison. PlatformIO or system Python results must
not be mixed into the same equivalence claim.
