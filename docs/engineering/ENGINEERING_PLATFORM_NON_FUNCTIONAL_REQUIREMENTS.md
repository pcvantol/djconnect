...
| NFR-LOC-001 | All operator-visible EP copy, including dynamic phase/status labels, errors, installer copy and localized accessibility names, ships for `en`, `nl`, `de`, `fr` and `es`. Raw identifiers/localization keys are never displayed. | Locale-key parity and browser catalogue-inventory checks. |
...
`2.0.0`, not through a DJConnect developer-host bootstrap.
