...
Release manifest and seven successfully completed target-scoped operations. The
...
`apple_private_device/iphone` with required paired-Watch validation, and
`windows_internal_arm64`. Their exact workflow evidence is recorded in
`PLATFORM_3_3_CURRENT_MAIN_MANIFEST_PROPOSAL.json` and the target completion
records. Home Assistant and the required iPad target remain open. No complete
...
2. obtain a target-scoped authorization and complete manifest-bound deployment
   plus smoke for the required iPad target; and
...
`PLATFORM_RELEASE_3_3_MANIFEST_APPROVED_PARTIAL_DEPLOYMENT_OPERATIONAL` pending
the two remaining target-specific operations and later burn-in evidence.

## Current candidate reconstruction

The approved current manifest is
`release-3.3.0-internal-20260714`, status
`APPROVED_PARTIAL_DEPLOYMENT_OPERATIONAL`. Its seven completed target operations
