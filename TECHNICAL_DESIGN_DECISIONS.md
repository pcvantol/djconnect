...
- Persisting a rotated refresh token must not reload the config entry. The
  runtime keeps the latest token and access-token cache in memory so normal
  client status polling does not become a refresh-token/reload loop.
- Persisting `last_device_status` is also a runtime cache update and must not
...
- Current-artist genre enrichment is non-critical. If Spotify returns an error
  such as HTTP 429, DJConnect backs off that lookup while keeping playback and
  status responses available.
...
- Runtime cache persistence should survive Home Assistant restarts but should
  not interrupt live playback/status polling with config-entry reloads.
...
- App-client pairing values generated by Home Assistant, such as the app
  pairing code and Home Assistant URL, are read-only description placeholders in
  config-flow. App clients submit their identity and device name back to HA
  through the pairing endpoint.
...
- The Spotify repair path opens OAuth and only closes after a new token is
  stored.
- After the Spotify callback stores a new token, the repair flow returns the
  completed entry directly instead of leaving an extra external-step popup open.
