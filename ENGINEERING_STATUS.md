...
PR [#923](https://github.com/pcvantol/djconnect/pull/923) is finalized by its
...
`main`. Its historical rolling-record lag is reconciled without recreating
Prompt History.
...
`main`. Their bounded dashboard corrections preserve raw audit evidence and
all lifecycle and authority semantics.
...
and is contained in current `main`. It adds read-only localized open-PR
readiness projection and bounded 30-second polling only while checks run. This
dedicated governance-only Finalization is the only permitted follow-up while
the repository is `MERGED_UNRECONCILED`; its merge restores Repository State
`MERGED_RECONCILED` and Workspace State `WORKSPACE_READY` after cleanup. No
lifecycle, validation, reviewer, provider, queue, delivery or merge-authority
semantics change.
