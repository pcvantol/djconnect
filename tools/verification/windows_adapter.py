"""Thin Windows runtime adapter for verification execution."""
...
class WindowsAdapterConfig:
    target: WindowsRuntimeTarget | None = None
    ...
    @classmethod
    def from_environment(cls, root: Path | None = None) -> "WindowsAdapterConfig":
        target_json = os.getenv("DJCONNECT_VERIFICATION_WINDOWS_TARGET_JSON", "")
        target = WindowsRuntimeTarget.from_mapping(json.loads(target_json)) if target_json else None
        evidence_dir = _resolve_optional(root, os.getenv("DJCONNECT_VERIFICATION_WINDOWS_EVIDENCE_DIR"))
        return cls(
            target=target,
            timeout_seconds=float(os.getenv("DJCONNECT_VERIFICATION_WINDOWS_TIMEOUT", "30")),
            allow_destructive=_truthy(os.getenv("DJCONNECT_VERIFICATION_ALLOW_DESTRUCTIVE")),
            allow_live_remote=_truthy(os.getenv("DJCONNECT_VERIFICATION_WINDOWS_ALLOW_REMOTE")),
            evidence_dir=evidence_dir,
        )


class WindowsVerificationAdapter(VerificationAdapter):
    """Execute Windows runtime primitives without scenario assertions."""

    name = "windows_native_arm64"

    def __init__(
        self,
        config: WindowsAdapterConfig | None = None,
        *,
        runner: Any | None = None,
    ) -> None:
        self.config = config or WindowsAdapterConfig.from_environment()
    ...
    def click(self, target: str, **kwargs: Any) -> PrimitiveResult:
        return PrimitiveResult(
            ...
            message="Windows UI input requires a configured UI automation driver.",
        )
    ...
    def type(self, text: str, **kwargs: Any) -> PrimitiveResult:
        return PrimitiveResult(
            ...
            message="Windows UI text input requires a configured UI automation driver.",
        )
    ...
    def collect_logs(self) -> tuple:
        ...
        if target and target.log_command:
            ...
            logs.append({"source": "windows_log_command", "ok": result.ok, "data": result.data})
    ...
    def capture_screenshot(self, name: str | None = None) -> PrimitiveResult:
        ...
        path = evidence_dir / f"{_safe_name(name or target.target_id or 'windows-target')}.png"
    ...
    def collect_environment(self) -> dict[str, Any]:
        ...
        return {
            ...
            "client_repository": "pcvantol/djconnect-windows",
            "target": self._target_dict(),
            "target_metadata": metadata.data,
            "capabilities": {
                "target_configured": self.config.target is not None,
                "local_runtime": self.config.target is not None and self.config.target.runtime == "local",
                "remote_runtime": self.config.target is not None and self.config.target.runtime == "remote" and self.config.allow_live_remote,
    ...
    def validate_target_identity(self, expected: dict[str, Any] | None = None) -> PrimitiveResult:
        target = self.config.target
        if target is None:
            return PrimitiveResult("validate_target_identity", False, {"error": "WindowsTargetUnavailable"})
        if target.runtime == "remote" and not self.config.allow_live_remote:
            return PrimitiveResult("validate_target_identity", False, {"error": "LiveWindowsRemoteExecutionNotConfigured", "target": target.to_dict()})
        if target.runtime == "remote" and not target.host:
            return PrimitiveResult("validate_target_identity", False, {"error": "WindowsRemoteTargetIncomplete", "target": target.to_dict()})
        if target.runtime not in {"local", "remote"}:
    ...
    def collect_runtime_metadata(self) -> PrimitiveResult:
        target = self.config.target
        if target is None:
            return PrimitiveResult("collect_runtime_metadata", False, {"error": "WindowsTargetUnavailable"})
        if target.metadata_command:
            return self._run_target_command(target.metadata_command, "collect_runtime_metadata", timeout=10)
        return PrimitiveResult("collect_runtime_metadata", True, {"target": target.to_dict(), "client_repository": "pcvantol/djconnect-windows"})

    def launch_app(self) -> PrimitiveResult:
        target = self.config.target
        if target is None:
            return PrimitiveResult("launch_app", False, {"error": "WindowsTargetUnavailable"})
    ...
    def stop_app(self) -> PrimitiveResult:
        target = self.config.target
        if target is None:
            return PrimitiveResult("stop_app", False, {"error": "WindowsTargetUnavailable"})
    ...
    def _run_target_command(self, command: str, operation: str, *, timeout: int | None = None) -> PrimitiveResult:
        target = self.config.target
        if target is None:
            return PrimitiveResult(operation, False, {"error": "WindowsTargetUnavailable"})
...
def _safe_name(value: str) -> str:
    ...
    return cleaned.strip("-") or "windows-target"
