...
#codexChat .codex-chat__details>.estimate-meta{padding-right:0}#downloadChat.download--glyph{align-items:center;background:#34283f;border:1px solid #d0a4ff;border-radius:8px;color:#eadcff;display:inline-flex;float:right;font-size:12px;gap:5px;height:36px;justify-content:center;min-height:36px;min-width:auto;padding:0 10px;position:relative;right:auto;top:auto;width:auto;z-index:auto}#downloadChat.download--glyph::after{content:"Download";font:600 12px/1 system-ui}#downloadChat.download--glyph:hover{background:#4b3658}#downloadChat.download--glyph:focus-visible{box-shadow:0 0 0 4px #d0a4ff40;outline:2px solid #d0a4ff;outline-offset:2px}
...
:is(.theme-toggle,.section-state-toggle)[aria-checked="true"]::before{background:#f0b66a}.auto-refresh-toggle{font:12px system-ui;min-height:44px;min-width:44px}.auto-refresh-toggle input{appearance:none;background:#4a4a55;border:0;border-radius:999px;cursor:pointer;height:24px;margin:0;position:relative;transition:background .18s ease;width:42px}.auto-refresh-toggle input::after{background:#f7f3ee;border-radius:50%;box-shadow:0 1px 3px #0008;content:"";height:18px;left:3px;position:absolute;top:3px;transform:translateX(0);transition:transform .18s ease;width:18px}.auto-refresh-toggle input:checked{background:#f0b66a}.auto-refresh-toggle input:checked::after{transform:translateX(18px)}.auto-refresh-toggle input:focus-visible{box-shadow:0 0 0 5px color-mix(in srgb,#f0b66a 24%,transparent);outline:3px solid #f0b66a;outline-offset:3px}html[data-theme="light"] .auto-refresh-toggle input{background:#4a4a55}html[data-theme="light"] .auto-refresh-toggle input:checked{background:#f0b66a}
...
function addCategoryDescriptions(){const descriptions=[[".workspace-card","De actieve werkruimte van dit project."],["#queueItems","Prompts worden uitgevoerd op volgorde van aanmaakdatum."],["#promptHistory","Alle terminale Engineering Platform-uitvoeringen, lokaal gecachet in de Engineering SQLite-opslag."],["#rateLimits","Beschikbare gebruiksruimte en resets van de actieve AI-provider."],["#lastExecutionGroup","De meest recent uitgevoerde prompt, met bewijs, rapport en analyse."],["#componentLogs","Geredigeerde, roterende logs van watcher en dashboard. Automatisch bijgewerkt via serverpush."],["#codexChat","Alleen lezen · context: repository, laatst uitgevoerde prompt en rapport. Deze chat kan geen engineering starten of wijzigingen uitvoeren."],["#engineering-dashboard-content>.technical-details:not(#componentLogs)","Operationele details over pull requests, repository, werkruimte en diagnose."]];for(const [selector,text] of descriptions){const category=document.querySelector(selector),summary=category?.querySelector(":scope>summary");if(!category||!summary)continue;let description=category.querySelector(":scope>.category-description")||category.querySelector(":scope>.estimate-meta");if(!description){description=document.createElement("p");description.textContent=text;summary.insertAdjacentElement("afterend",description)}description.classList.add("category-description")}}addCategoryDescriptions();
