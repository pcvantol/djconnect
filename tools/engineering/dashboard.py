...
<details class="card" id="report"><summary><strong>Engineeringrapport</strong></summary><div class="field"><span class="label">Markdownrapport</span><pre id="reportContent">Open dit blok om het rapport te laden.</pre></div></details>
...
let reportLoaded=false;function report(){if(reportLoaded)return;reportLoaded=true;fetch("/api/report/latest").then(x=>x.text()).then(x=>$("reportContent").textContent=x).catch(()=>{$("reportContent").textContent="Engineeringrapport is niet beschikbaar."})}
...
let e=new EventSource("/api/events");e.addEventListener("status",x=>{try{r(JSON.parse(x.data));humanize()}catch{r(fallback);humanize()}});$("report").addEventListener("toggle",()=>{$("report").open&&report()});setInterval(()=>{clock();humanize();if(nextRefresh&&Date.now()>=nextRefresh)refresh()},250);clock();refresh();setInterval(promptStarted,5000);promptStarted()
</script>"""
