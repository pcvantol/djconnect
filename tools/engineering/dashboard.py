...
function addCategoryDescriptions(){const descriptions=[[".workspace-card","De actieve werkruimte van dit project."],["#queueItems","Prompts in uitvoervolgorde: oudste eerst. Ook een lege wachtrij blijft zichtbaar."],["#promptHistory","Alle terminale Engineering Platform-uitvoeringen, lokaal gecachet in de Engineering SQLite-opslag."],["#rateLimits","Beschikbare gebruiksruimte en resets van de actieve AI-provider."],["#lastExecutionGroup","De meest recent uitgevoerde prompt, met bewijs, rapport en analyse."],["#componentLogs","Geredigeerde, roterende logs van watcher en dashboard. Automatisch bijgewerkt via serverpush."],["#codexChat","Alleen lezen · context: repository, laatst uitgevoerde prompt en rapport. Deze chat kan geen engineering starten of wijzigingen uitvoeren."],["#engineering-dashboard-content>.technical-details:not(#componentLogs)","Operationele details over pull requests, repository, werkruimte en diagnose."]];for(const [selector,text] of descriptions){const category=document.querySelector(selector),summary=category?.querySelector(":scope>summary");if(!category||!summary)continue;let description=category.querySelector(":scope>.category-description")||category.querySelector(":scope>.estimate-meta");if(!description){description=document.createElement("p");description.textContent=text;summary.insertAdjacentElement("afterend",description)}description.classList.add("category-description")}}addCategoryDescriptions();
