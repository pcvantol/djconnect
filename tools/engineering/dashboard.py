...
strong{color:#c7a6ff}.status{display:flex;align-items:center;gap:8px}.queue-list{display:grid;gap:7px;list-style:none;margin:10px 0 0;padding:0}.queue-item{align-items:start;border-top:1px solid #3d3651;display:grid;gap:8px;grid-template-columns:2rem minmax(0,1fr);padding-top:7px}.queue-item:first-child{border-top:0;padding-top:0}.queue-item__number{color:var(--category-color);font:700 12px ui-monospace,monospace;padding-top:1px;text-align:right}.queue-item__title{display:block;font-weight:600}.queue-item__meta{color:#b9b6c0;font:12px ui-monospace,monospace;margin-top:2px}.queue-empty{color:#b9b6c0;font-size:12px}.indicator{width:12px;height:12px;border-radius:50%;background:#9a9aa3;box-shadow:0 0 8px #9a9aa388;flex:none}.indicator--green{background:#51d88a;box-shadow:0 0 8px #51d88a88}.indicator--yellow{background:#f4d35e;box-shadow:0 0 8px #f4d35e88}.indicator--orange{background:#ff9f43;box-shadow:0 0 8px #ff9f4388}.indicator--red{background:#ff6b6b;box-shadow:0 0 8px #ff6b6b88}.indicator--running{background:transparent;border:3px solid #ff6b6b;border-right-color:transparent;box-sizing:border-box;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
...
<footer class="footer"><span class="label">Engineering Platform-versie</span><span id="platformVersion">Laden…</span> · <span class="label">Git-commit</span><code>$BUILD_COMMIT</code></footer><span id="dashboardVersion" hidden></span><span id="workerVersion" hidden></span>
<script>
...
function copyReport(){report().then(()=>copyText($("reportContent").textContent)).then(showCopyToast).catch(()=>{$("copyReport").textContent="Kopiëren mislukt"})}
function copyReportAnalysis(){analysis().then(()=>copyText($("reportAnalysisContent").textContent)).then(showCopyToast).catch(()=>{$("copyReportAnalysis").textContent="Kopiëren mislukt"})}
...
function addCategoryDescriptions(){const descriptions=[[".workspace-card","De actieve werkruimte en de locatie van dit dashboard."],["#queueItems","Prompts in uitvoervolgorde: oudste eerst. Ook een lege wachtrij blijft zichtbaar."],["#promptHistory","Alle terminale Engineering Platform-uitvoeringen, lokaal gecachet in de Engineering SQLite-opslag."],["#rateLimits","Beschikbare gebruiksruimte en resets van de actieve AI-provider."],["#lastExecutionGroup","De meest recent uitgevoerde prompt, met bewijs, rapport en analyse."],["#componentLogs","Geredigeerde, roterende logs van watcher en dashboard. Automatisch bijgewerkt via serverpush."],["#codexChat","Alleen lezen · context: repository, laatst uitgevoerde prompt en rapport. Deze chat kan geen engineering starten of wijzigingen uitvoeren."],["#engineering-dashboard-content>.technical-details:not(#componentLogs)","Operationele details over pull requests, repository, werkruimte en diagnose."]];for(const [selector,text] of descriptions){const category=document.querySelector(selector),summary=category?.querySelector(":scope>summary");if(!category||!summary)continue;let description=category.querySelector(":scope>.category-description")||category.querySelector(":scope>.estimate-meta");if(!description){description=document.createElement("p");description.textContent=text;summary.insertAdjacentElement("afterend",description)}description.classList.add("category-description")}}addCategoryDescriptions();
