...
function groupLastExecution(){const group=$("lastExecution")?.parentElement;if(!group||group.classList.contains("last-execution-group"))return;group.classList.add("last-execution-group");const heading=document.createElement("h2");heading.className="last-execution-group__heading";heading.textContent="Laatste uitgevoerde prompt";group.prepend(heading)}groupLastExecution();
renderChatHistory();
</script>"""
    return (
