...
#componentLogs,#componentLogs .card{background:#302a24;border-left:3px solid #f0b66a}main>details.technical-details:not(#componentLogs),main>details.technical-details:not(#componentLogs) .card{background:#28263a;border-left:3px solid #a78bfa}
</style>
<h1>$TITLE</h1>
...
function addProviderEvidence(){for(const [rootId,targetSelector,fieldId] of [["currentRun",".current-run__title","currentProvider"],["lastExecution",".card--previous","lastProvider"]]){const root=$(rootId),target=root?.querySelector(targetSelector);if(!target||$(fieldId))continue;const field=document.createElement("div"),label=document.createElement("span"),value=document.createElement("span");field.className="field";field.id=fieldId;label.className="label";label.textContent="AI-provider";value.textContent="Codex CLI";field.append(label,value);target.append(field)}}addProviderEvidence();
renderChatHistory();
</script>"""
    return (
