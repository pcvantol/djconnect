...
| Component Release Mode | `GO_COMPONENT_RELEASE_SCOPE_REFINEMENT_PARTIALLY_QUALIFIED` | `docs/release/COMPONENT_RELEASE_SCOPE_REFINEMENT.md` defines one deterministic closure contract and profiles HACS, API, website, ESP32, iOS/watchOS, macOS, Windows and the shared Pi renderer family. Pi 4-inch and Pi 10-inch remain non-selectable because they have no independent artifact identity. | Implement Runtime-enforced selected-component and exact dependency/evidence closure; current dispatchers remain dry-run only. | Future Implementation | Platform Evolution / Platform Release | Component Release Selection and Evidence Closure implementation, followed only by profile-specific execute qualification and a real bounded patch. |
