...
| Component Release Mode | `GO_COMPONENT_RELEASE_MODE_PARTIALLY_QUALIFIED` | Existing Repository Ownership, Platform Release Architecture, manifest/runtime and the HACS 3.3.1 component-patch record identify canonical release units and their patch, evidence and governance boundaries. | Prove a generic fail-closed one-component selection path using the existing manifest/runtime: one source participant, necessary distribution/target dependency, patch-only handling, affected-only evidence, qualified runner routing and recovery posture. | Future Assessment | Platform Evolution / Platform Release | Component Release Qualification assessment, when one existing ownership participant and current manifest/runtime evidence are selected. |
| TD-GITHUB-001 — GitHub Actions retention and evidence preservation | `GO_TD_GITHUB_001_PARTIALLY_QUALIFIED` | Existing release, post-merge, smoke, Verification, Software Assurance and Golden-report contracts classify Permanent, Long-term, Short-term and Ephemeral evidence. | Qualify that every decision-bound Permanent/Long-term item has a redacted, immutable and independently durable record despite Actions run cleanup; missing preservation must fail closed for the affected decision. | Future Assessment | Platform Evolution / Software Assurance / Platform Release | Evidence Preservation Qualification, when current retention and durable-record evidence can be reviewed without changing Actions configuration. |
