...
Use it when you want a voice/PTT music remote that can start playback, answer Ask DJ music questions, list available speakers, offer Play Now actions and deliver mood-aware DJ announcements through the DJConnect client instead of a generic speaker. ESP32 and Raspberry Pi stay local-only; iOS, macOS and Windows can use a Home Assistant remote URL after local pairing.

With Spotify Direct, Ask DJ can answer recent Spotify listening questions for tracks, albums, artists and playlist contexts with compact artwork/icon lists. Music-backend playback state stays internal to DJConnect and its clients instead of being exposed as separate Spotify/Music Assistant playback entities in Home Assistant. With Music Assistant, DJConnect controls one configured Music Assistant player and leaves provider login, library, queues and grouping/sync to Music Assistant. DJConnect no longer creates a native playback proxy media player.
