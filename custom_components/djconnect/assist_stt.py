...
NO_STT_PROVIDER = "No STT provider configured. Checked Assist pipeline/default STT."


class DJConnectSttError(RuntimeError):
    """Base error for DJConnect STT handling."""


class DJConnectNoSttProviderError(DJConnectSttError):
...
async def transcribe_wav_with_assist(
    ...
    _LOGGER.info(
        "DJConnect STT request: ha_version=%s pipeline_id=%s pipeline_name=%s "
    ...
    try:
        ...
        raise DJConnectNoSttProviderError(_no_stt_provider_message()) from exc
    ...
    _LOGGER.info(
        "DJConnect STT provider result: type=%s state=%s has_text=%s",
...
async def _transcribe_with_assist_pipeline(
    ...
    _LOGGER.info(
        "DJConnect STT fallback: using HA Assist audio pipeline helper "
    ...
    try:
        ...
        _LOGGER.warning("DJConnect STT fallback pipeline run failed: %s", exc)
        message = str(exc)
        if "speech-to-text" in message or "does not support" in message:
            raise DJConnectNoSttProviderError(_no_stt_provider_message()) from exc
        raise DJConnectSttError(f"HA Assist STT failed: {message}") from exc

    text = _text_from_pipeline_events(events)
    _LOGGER.info(
        "DJConnect Assist pipeline STT events: count=%s types=%s has_text=%s",
        len(events),
        _event_types(events),
        bool(text),
    )
...
def _resolve_stt_info(
    ...
    if not engine:
        ...
        if engine:
            ...
            _LOGGER.info(
                "DJConnect STT provider selected from HA stt entity fallback: %s",
                engine,
            )
    return SttInfo(
...
def _get_assist_pipeline(hass: HomeAssistant, pipeline_id: str | None) -> Any | None:
    try:
        ...
        _LOGGER.warning("DJConnect Assist pipeline registry unavailable: %s", exc)
        return None
    ...
    if pipeline_id:
        ...
        if pipeline is not None:
            return pipeline
        _LOGGER.warning(
            "DJConnect configured Assist pipeline %s was not found; falling back "
    ...
    if callable(current_getter):
        try:
            ...
            _LOGGER.warning("DJConnect preferred Assist pipeline lookup failed")
...
async def _process_with_stt_engine(
    ...
    if callable(getter):
        ...
        if provider is None:
            raise DJConnectNoSttProviderError(f"STT provider not found: {engine}")
        checker = getattr(provider, "check_metadata", None)
        if callable(checker) and not checker(metadata):
            _LOGGER.warning(
                "DJConnect STT provider %s does not support metadata %s",
                engine,
                metadata,
            )
        ...
        raise DJConnectNoSttProviderError(f"STT provider cannot process audio: {engine}")
    ...
    raise DJConnectNoSttProviderError(_no_stt_provider_message())
...
def _wav_parameters(wav: bytes) -> tuple[int, int, int]:
    ...
    try:
        ...
        _LOGGER.debug("DJConnect could not parse WAV header; using default STT metadata")
        return 16000, 1, 2
...
def _require_text(value: Any) -> str:
    ...
    if not text:
        raise DJConnectSttError("HA Assist STT did not return recognized text")
    return text
