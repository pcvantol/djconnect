...
- Home Assistant integration: `3.2.12`
- Domain: `djconnect`
- HACS category: `Integration`
- Device target: DJConnect device
- Firmware mDNS service: `_djconnect._tcp`
- Device ID format: `djconnect-lilygo-t-embed-s3-XXXXXXXXXXXX`.
- Legacy `djconnect-XXXXXXXXXXXX` device IDs are not accepted.

## Features

- Pair a DJConnect device with the displayed 6-digit pairing code.
...
- **First-class opt-in Music DNA**: lightweight clients do not store Music DNA.
  Home Assistant owns compact Music DNA through runtime session memory plus
  Home Assistant Store key `djconnect_music_dna` version `1`, but only builds
  knowledge after the user/client context opts in. Memory is scoped first by HA
  user id when available, then by DJConnect client/device id, so a Watch request
  such as `Draai iets rustigers` can be followed later from another client with
  `Waarom koos je dit?`. Stored memory excludes OAuth tokens, bearer tokens, raw
  audio and full prompts. Clients can use
  `POST /api/djconnect/music_dna/profile`,
  `/api/djconnect/music_dna/settings` and
  `/api/djconnect/music_dna/clear` to render the Music DNA screen, set opt-in
  and clear learned DNA at any time.
...
- **Optional local websocket fast path**: app clients may use Home Assistant's
  ...
  `djconnect/ask_dj/message`, `djconnect/ask_dj/idle_suggestion`,
  `djconnect/track_insight` and
  `djconnect/music_dna/{profile,settings,clear}`; payloads use the same
  contracts as their HTTP equivalents and still include the DJConnect
...
- **Split local-device and app pairing**: ESP32 and Raspberry Pi remain LAN/local-device clients with `_djconnect._tcp`, optional Client adres fallback and `/api/device/*` endpoints for device actions. iPhone/iPad, Apple Watch, macOS and Windows are inbound-only app clients: Home Assistant generates their pairing code, iPhone/iPad scans the iPhone/iPad QR-code image/deep-link payload, Apple Watch pairs through a separate iPhone/iPad-scanned Watch proxy QR-code image/deep-link payload, and macOS/Windows enter the local Home Assistant URL plus code manually. App clients call Home Assistant through `/api/djconnect/...`, and remote-capable apps can use `ha_remote_url` when Home Assistant has an external/Nabu Casa URL. Home Assistant never discovers or calls an app-hosted local API. Each client type has a stable ID prefix (`djconnect-ios-*`, `djconnect-macos-*`, `djconnect-watchos-*`, `djconnect-raspberry-pi-*`, `djconnect-windows-*` or model-specific ESP32 IDs) and Home Assistant rejects mismatched `client_type` metadata.
- **Client-specific capability surfaces**: ESP32 gets hardware/runtime entities such as firmware update, reboot, battery, Wi-Fi RSSI, display, LED, wake word and device language. Raspberry Pi keeps local-device pairing plus Pi-specific restart/shutdown controls and kiosk/display use cases. iOS, macOS, watchOS and Windows get app/runtime, backend playback, Ask DJ, APNs readiness where applicable and remote-capable API surfaces where supported, but no ESP firmware controls. Assist Conversation Agent-only entries load only the `DJConnect DJ` conversation agent and minimal diagnostics.
...
- **No secrets in diagnostics/logs**: diagnostics and debug logs redact keys
  containing `token`, `password`, `secret`, `proof`, `authorization`, `prompt`,
  `history`, `memory` or `raw_audio`; logs avoid full ESP payloads and do not
  intentionally log Spotify refresh tokens, WiFi passwords, device tokens, raw
  prompts, raw audio or Ask DJ history.
...
iPhone/iPad/Spotify OAuth testing are documented in
...
- A supported music backend:
  - Spotify Direct requires Spotify Premium and a user-owned Spotify Developer
    app Client ID.
  - Music Assistant requires Music Assistant installed/configured in Home
    Assistant with at least one usable player and handles provider login inside
    Music Assistant, not DJConnect.
...
- A DJConnect ESP32 device, iOS/macOS/watchOS app, Raspberry Pi client or
  Windows client on the same local network as Home Assistant during pairing.
- For ESP32 devices: 2.4 GHz WiFi.
- For Spotify Direct OAuth: an external HTTPS Home Assistant URL, preferably
  Nabu Casa. This is not required by DJConnect when you choose Music Assistant.
...
8. Choose a backend:
   - **Spotify Direct** opens DJConnect's Spotify OAuth step in Home Assistant.
   - **Music Assistant** asks only for a Music Assistant target player and skips
     DJConnect Spotify Client ID/OAuth fields.
...
Home Assistant reads the bundled DJConnect icon and logo from
`custom_components/djconnect/brand/`. If the integration still shows a generic
icon after an update, redownload DJConnect in HACS, restart Home Assistant and
refresh the browser/app cache.

## Choose A Music Backend

DJConnect has two explicit backend routes:

- **Spotify Direct** is the default DJConnect-owned Spotify route. Use it when
  you want DJConnect to authorize Spotify directly from Home Assistant and use
  Spotify Web API features such as playback devices, queue, playlists,
  favorites, recent listening history, top items, profile analysis and
  Spotify-backed recommendations.
- **Music Assistant** is the no-DJConnect-Spotify-OAuth route. Use it when you
  already run Music Assistant in Home Assistant and want DJConnect to control one
  configured Music Assistant `media_player`. Music Assistant owns provider
  authentication, library browsing, queues and grouping/sync. DJConnect stores
  only the selected target player and uses capability fallbacks for Spotify-only
  features.

You can switch later from the DJConnect options flow with `Muziekbackend
wijzigen` / `Change music backend`. Pairing, device tokens, Ask DJ history, DJ
Memory and push registrations stay in place. DJConnect increments
`music_backend_revision` so clients can discard old backend-specific Play Now
actions.
...
Spotify Direct is the default backend in `3.2.0`. Spotify Premium is required
for Spotify Direct because DJConnect asks Home Assistant to control Spotify
playback devices through Spotify's playback APIs. Spotify OAuth credentials
remain in Home Assistant; DJConnect devices and apps never receive the Spotify
refresh token or playback credentials.

If you choose Music Assistant instead, skip this whole Spotify Developer app and
OAuth section. DJConnect will not ask for a Spotify Client ID, will not open the
Spotify OAuth website, and will not create Spotify OAuth Repair issues while
Music Assistant is the selected backend.
...
step when you choose Spotify Direct. With Music Assistant, provider
authorization and library access stay in Music Assistant and DJConnect controls
the selected Music Assistant player entity.

1. Choose one setup route: `Assist Conversation Agent`, `DJConnect lokaal device koppelen` or `DJConnect app koppelen`.
2. Use `Assist Conversation Agent` for Home Assistant Assist satellites without a DJConnect client pairing code, device token or Client adres.
3. Use `DJConnect lokaal device koppelen` for ESP32 or Raspberry Pi clients. These are LAN-only devices; mDNS is preferred and Client adres is only an advanced/fallback local URL.
4. Use `DJConnect app koppelen` for iPhone/iPad, Apple Watch, macOS or Windows. The next step first asks for the client type, then Home Assistant shows only the relevant pairing fields. Home Assistant generates the app pairing code and inline QR-code images. iPhone/iPad scans the iPhone/iPad QR/deep-link; Apple Watch pairs through a separate Watch proxy QR/deep-link scanned on the iPhone/iPad so the Watch does not need HA URL entry; macOS and Windows enter the local Home Assistant URL plus code manually. The app must be on the LAN for pairing and posts its pairing request to `/api/djconnect/pair`; Home Assistant does not discover or call a client-hosted `/api/device/*` API.
5. For ESP32 BLE WiFi setup, choose one BLE action: write WiFi over Bluetooth, rescan Bluetooth devices, or continue directly to pairing when WiFi was already configured through the device captive portal.
6. For local ESP32/Raspberry Pi pairing, enter the 6-digit pairing code shown by the device.
7. Confirm the HTTPS Home Assistant external URL. DJConnect prefills this from Home Assistant's Network external URL when available and falls back to the Nabu Casa/Home Assistant Cloud remote UI URL. Remote-capable apps can receive this as `ha_remote_url` after local pairing.
8. If you chose Spotify Direct, copy the shown redirect URI into your Spotify Developer app and enter that app's Spotify Client ID.
9. If you chose Spotify Direct, Home Assistant opens the Spotify authorization website.
10. If you chose Spotify Direct, approve access in Spotify.
11. If you chose Music Assistant, select the Music Assistant player DJConnect should control.
12. Return to Home Assistant and complete the voice/DJ settings step.
...
Where Home Assistant exposes choices, DJConnect shows populated dropdowns for Assist pipeline. The first setup step chooses one route and the next pairing step chooses the client type before any pairing details are shown. App pairing offers iPhone/iPad, Apple Watch, macOS and Windows and hides Client adres. iPhone/iPad and Apple Watch pairing show inline QR-code images with their deep-link payloads on a white QR background; Apple Watch pairing is initiated by scanning the Watch proxy QR/deep-link on the iPhone/iPad, so the Watch does not require manual HA URL entry. Local-device pairing offers ESP32 and Raspberry Pi and keeps Client adres available only as a local fallback. Intermediate config/options forms are marked as non-final so Home Assistant can present next/back navigation until the final submit screen. STT/TTS engine, language and voice are managed in Home Assistant Assist, not in DJConnect. Backend playback is handled by Home Assistant through DJConnect commands plus Spotify-backed control/status entities; ESP32 and Raspberry Pi device settings use the local device command API. The compact options screen used from Assist conversation-agent settings shows only the action selector; DJ announcement style is no longer a config/option choice because runtime mood drives the final tone when available. Device-only setup fields such as Client adres, Assist pipeline, firmware channel, playlist overrides, Spotify source overrides and OTA/audio compatibility fields are hidden there. Max audio bytes, OTA battery settings and DJ announcement audio TTL use integration defaults and are not user-adjustable in config/options flow. Firmware OTA device selection is ESP32-only and automatic: DJConnect reads the public multi-device firmware manifest and selects the matching `firmwares[]` entry from ESP status/info, falling back to LilyGO only before the ESP has reported a model. iOS/macOS clients update through TestFlight/app distribution, Windows through its own app distribution/source flow, and Linux/Raspberry Pi clients through their own GitHub source/install flow rather than ESP OTA.
...
- Pairing is unauthenticated by design, but requires the 6-digit pairing code shown on the DJConnect device.
...
- Logs avoid full event payloads and do not intentionally log Spotify refresh
  tokens, WiFi passwords, device tokens, raw prompts, raw audio or Ask DJ
  history.
...
Apple clients additionally expose APNs readiness diagnostics and a privacy-safe
test-push button. Raspberry Pi clients additionally expose restart and shutdown
buttons for their local client API. Assist Conversation Agent-only entries load
only the `DJConnect DJ` conversation agent plus the minimal diagnostics sensor,
so they do not create playback, device-control, firmware or push-action
entities.
...
- `djconnect.music_backend_status`
...
DJConnect setup uses an explicit backend choice:

- `Spotify Direct`: DJConnect owns Spotify PKCE OAuth inside Home Assistant and
  keeps full Spotify-backed playback, search, queue, playlist, output,
  favorites, recommendations and listening-profile features.
- `Music Assistant`: Music Assistant owns provider authentication and
  DJConnect only targets a configured MA `media_player`. DJConnect does not ask
  for a Spotify Client ID, does not run Spotify OAuth and does not create
  Spotify reauthorization repairs for this backend.
...
the existing Spotify OAuth/PKCE flow when credentials are missing.

Music Assistant support is intentionally an adapter, not a bundled Music
Assistant clone: DJConnect does not implement a provider registry, library
index, queue engine or grouping/sync engine. Unsupported features such as
Spotify recent listening history, top items, recommendations, favorites and
advanced queue/library profile views return capability-based fallback messages
instead of trying to emulate Spotify Direct.

`djconnect.test_parse` and `djconnect.test_command` use this flow:

```text
text -> HA Assist conversation pipeline -> DJConnect intent -> selected music backend -> ESP DJ announcement
```

`djconnect.test_command` accepts `command_text` and optional `play`. The legacy `text` key is still accepted for existing YAML/scripts. With `play: false`, it uses the same command parser path without starting playback on the selected backend.
...
Assist fuzzy-correction step used by physical PTT, then music intent parsing,
selected-backend playback/search where supported, DJ announcement generation, TTS audio
creation and delivery back to the connected DJConnect device/client.

`djconnect.music_backend_status` returns the selected backend, Music Assistant
target player, backend capabilities, revision and safe availability/error
...
`language`; app clients and Raspberry Pi determine their own UI language
locally. `client_type` identifies the paired DJConnect client runtime; current
values are `esp32`, `ios`, `macos`, `watchos`, `raspberry_pi` and `windows`.
ESP32 and Raspberry Pi traffic always uses `ha_local_url`; cloud/Nabu Casa URLs
are never sent to those local devices. iOS, macOS and Windows may receive
`ha_remote_url` in pairing/status responses when Home Assistant exposes an
external HTTPS URL, and the app chooses local or remote at runtime. Spotify
OAuth credentials stay in Home Assistant and are used only by the HA playback
backend. Pair/status payloads must not contain `ha_url`, `refresh_token`,
...
resolved `music_dna_key`. Clients should treat Music DNA as server-side state,
must not store Spotify credentials, Home Assistant tokens or Music DNA locally,
and should render backend profile data from
`POST /api/djconnect/music_dna/profile` instead of deriving a persistent profile
from local Track Insight history. Music DNA is disabled until
`POST /api/djconnect/music_dna/settings` stores `enabled:true`; while disabled,
Home Assistant returns `enabled:false` and `profile:{}` and does not build new
Music DNA knowledge. `POST /api/djconnect/music_dna/clear` clears learned DNA
without changing the opt-in setting, so an enabled profile starts learning again
from empty data.
...
over `language` and store it as `provision.language`. App clients may also use
an optional `ha_remote_url` when present; ESP32 and Raspberry Pi must ignore and
must not receive remote URLs.
...
announcements may also use compact Music DNA for a short personal opening line,
such as welcoming the user back or referencing the daypart. DJConnect does not
use arbitrary Home Assistant entity state for announcement personalization.
...
Required headers are `Authorization: Bearer <device_token>`, `X-DJConnect-Device-ID` and `Content-Type: application/json`. Supported commands include `status`, `devices`, `queue`, `playlists`, `pause`, `play`, `next`, `previous`, `seek_relative`, `start_liked_proxy`, `start_playlist`, `play_context_at`, `ask_dj_play_recommendation`, `set_shuffle`, `set_repeat`, `set_output`, `set_volume`, `volume_delta`, `save_current_track` and `set_current_track_favorite`. `seek_relative` accepts an integer millisecond offset for Apple app skip-forward/skip-back controls; positive values seek forward and negative values seek backward. `set_shuffle` accepts a boolean value; `set_repeat` accepts `off`, `track` or `context`; `volume_delta` accepts an integer relative volume step; `save_current_track` saves the current Spotify track to the user's Liked Songs/favorites through Spotify's library API and remains a legacy alias. `set_current_track_favorite` accepts a boolean `value`: `true` saves the current Spotify track to Liked Songs/favorites and `false` removes it. `play_context_at` accepts a context URI and track offset URI for Up Next playback. `ask_dj_play_recommendation` accepts the backend-aware Play Now `value` returned in `playback_actions[]`; Spotify Direct values may include `uri`, `uris[]`, `context_uri` and `offset_uri`, while Music Assistant values use `item_id`, `provider`, `media_type` and `target_player_id`. Clients should echo `backend` and `music_backend_revision`; HA rejects stale actions with `error:"stale_backend_action"`. Responses are generic JSON shapes with `playback`, `devices`/`outputs`, `queue` or `playlists`, so future backends can be added without firmware changes. `status` responses include `backend_available`, `ha_version`, `ha_major_minor`, `music_backend`, `music_backend_name`, `music_backend_available`, `music_backend_revision`, `music_backend_capabilities`, `music_target_player`, safe `music_backend_error` and a valid `playback` object even when no playback is active. Unsupported backend features return `success:false`, `error:"unsupported_backend_capability"`, `capability`, `backend` and a user-facing `message`. Current playback can include `is_liked`/`favorite_status` when Spotify library status is available, so clients can render the favorite toggle state. `queue` responses include at most 100 items, top-level `context_uri` / `contextUri` when known and per-item artwork aliases such as `album_image_url` and `image_url`; `playlists` responses include playlist aliases such as `name`, `title`, `display_title`, `uri`, `value`, `playlist_uri`, `owner`, `subtitle`, `image_url`, `entity_picture`, `album_image_url`, `album_art_url` and `media_image_url`. HA returns playlist lists as top-level `playlists` and `items`, plus `data.playlists`, `data.items`, `result.playlists`, `result.items` and `count` for stricter clients. ESP32 `playlists` requests may send `limit`; HA caps ESP32 responses at 20 items and returns up to 100 for app-like clients while paging provider APIs internally. A successful `playlists` response returns `backend_available:true` even when playback is idle; backend failures still return a non-empty JSON body with `success:false`, `backend_available:false` and empty playlist aliases. Logs never include device tokens, Spotify tokens or backend credentials.

HA and ESP firmware must share the same `major.minor` protocol version. Patch
versions may differ, so HA `3.0.x` can talk to ESP `3.0.y`, but HA `3.2.x`
...
Home Assistant expects ESP32 and Raspberry Pi local devices to expose:
...
`/api/device/*` is not part of the iOS, macOS, watchOS or Windows app contract. Apps are
inbound-only and call Home Assistant through `/api/djconnect/...`.
`/api/device/reboot` is ESP-specific. `/api/device/restart` and
`/api/device/shutdown` are Raspberry Pi-specific local client actions.
...
The integration uses the device `local_url` from pairing/status when provided.
During local-device setup it discovers visible ESP32/Raspberry Pi
`_djconnect._tcp` clients, probes `/api/device/pairing-info`, and can prefill
pairing fields for reachable local devices. Stale Bonjour records that no
longer answer pairing-info are hidden from the selector; use the manual Client
adres field only when mDNS is visible but the advertised local URL is wrong. If
the stored field is empty at runtime, it resolves the `_djconnect._tcp` service
for the paired local device. When the setup code is only 6 digits, DJConnect can
also use the single visible DJConnect local device on the network. Fallback
DJConnect never generates hostnames from a 6-digit pairing code. Legacy
`djconnect-[6-digit-code].local` fallbacks are intentionally ignored; use mDNS
discovery, device-reported `local_url`, or the advanced manual Client adres
field when needed.
...
```json
{
  ...
  "version_tag": "v3.2.12",
  "channel": "stable",
  "min_ha_integration": "3.2.12",
  "firmwares": [
    {
      "board": "t_embed_cc1101",
      "device": "lilygo-t-embed-s3",
      "asset": "djconnect-lilygo-t-embed-s3-v3.2.12.bin",
      "url": "https://github.com/pcvantol/djconnect-firmware/releases/download/v3.2.12/djconnect-lilygo-t-embed-s3-v3.2.12.bin",
...
```bash
./release.sh 3.2.12 --publish-firmware-repo ../djconnect-firmware
```
...
- Verify config/options-flow base, English and Dutch translation keysets stay
  aligned and have no stale `data_description` keys.
- Re-run security/diagnostics redaction checks when payloads, logs, diagnostics,
  Ask DJ history, memory, push registration or token handling changes.
...
```bash
git add .
git commit -m "Release DJConnect v3.2.12"
git tag v3.2.12
git push origin main
git push origin v3.2.12
gh release create v3.2.12 --title "DJConnect v3.2.12" --notes-file CHANGELOG.md
```
...
- Publish the LilyGO release asset, `djconnect-lilygo-t-embed-s3-vX.Y.Z.bin`.
...
- If provisioning says `local_url is unknown`, make sure the device advertises `_djconnect._tcp` mDNS or enter the Client adres reported by the device.
- If Home Assistant sees a Raspberry Pi local device through mDNS but pairing shows a pairing-info reachability error, verify that the Client adres shown in the pairing form opens `/api/device/pairing-info` from the Home Assistant network. Correct the Client adres manually if Bonjour advertised a hostname or port that Home Assistant cannot reach. iOS, macOS, watchOS and Windows app pairing does not use Client adres.
...
- If the ESP logs `HA status response: 401` while HA can still reboot the device, update to this release or newer and re-pair if needed. Status/command/voice auth accepts `djconnect-lilygo-t-embed-s3-XXXXXXXXXXXX` with the stored token, learns the current id, and keeps `ha_pairing_status` stable.
...
- Diagnostics are available from the Home Assistant integration page and redact
